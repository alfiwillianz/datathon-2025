...
def categorize_and_split_combined_jsonl(
    ...
    if not (train_ratio + test_ratio == 1.0):
        print("Error: Rasio pelatihan, dan pengujian harus berjumlah 1.0.")
        return
    ...
    print(f"\nLangkah 3: Membagi data secara stratifikasi menjadi set pelatihan, dan pengujian...")
    ...
    def split_list_stratified(data_list, train_r, test_r):
        total = len(data_list)
        train_s = int(total * train_r)
        
        train = data_list[:train_s]
        test = data_list[train_s:]
        return train, test

    train_code, test_code = split_list_stratified(codealpaca_samples, train_ratio, test_ratio)
    train_gsm, test_gsm = split_list_stratified(gsm8k_samples, train_ratio, test_ratio)
    train_other, test_other = split_list_stratified(other_samples, train_ratio, test_ratio)
...
if __name__ == "__main__":
    input_inference_jsonl = "Dataset/devstral_inference.jsonl"
    output_base_path = "Dataset/categorized_split_data_stratified"
