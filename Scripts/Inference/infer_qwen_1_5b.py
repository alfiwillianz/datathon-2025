...
def infer_qwen_1_5b_optimized():
    ...
    model_paths = {
        ...
        "adapter": "StudentModels/Adapters/qwen2.5-1.5b-lora/checkpoint-4122/"
    }
    
    # Check if paths exist
    if not os.path.exists(model_paths["base_model"]):
        logger.error(f"Base model path does not exist: {model_paths['base_model']}")
        return None
        
    if not os.path.exists(model_paths["adapter"]):
        logger.error(f"Adapter path does not exist: {model_paths['adapter']}")
        logger.info("Searching for available adapters...")
        adapter_base = "StudentModels/Adapters/"
        if os.path.exists(adapter_base):
            for item in os.listdir(adapter_base):
                logger.info(f"Found: {os.path.join(adapter_base, item)}")
        return None
